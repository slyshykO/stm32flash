...
#if defined(__WIN32__) || defined(__CYGWIN__)
BOOL __RPC_CALLEE CtrlHandler(DWORD fdwCtrlType)
{
    fprintf(stderr, "\nCaught signal %lu\n", fdwCtrlType);
    if (p_st && parser)
        parser->close(p_st);
...
int main(int argc, char* argv[])
{
    ...
    if (action == ACT_WRITE) {
        ...
        if (!force_binary) {
            ...
            if (!p_st) {
                fprintf(
                    stderr, "%s Parser failed to initialize\n", parser->name);
                goto close;
            }
        }
    ...
    fprintf(diag,
            "- RAM        : Up to %uKiB  (%ub reserved by bootloader)\n",
            (stm->dev->ram_end - 0x20000000) / 1024,
            stm->dev->ram_start - 0x20000000);
    fprintf(diag,
            "- Flash      : Up to %uKiB (size first sector: %ux%u)\n",
    ...
    if (action == ACT_READ) {
        ...
        if (num_pages != STM32_MASS_ERASE &&
            ...
            fprintf(stderr,
                    "Specified start & length are invalid (must be page "
                    "aligned)\n");
            ret = 1;
            goto close;
        }
        ...
        // if ((start % stm->dev->fl_ps[i]) != 0 || (end % stm->dev->fl_ps[i])
        // != 0) {
        ...
        // existing flash
        //       contents first, so it can be preserved and combined with new
        //       data
        ...
        again:
            ...
            if (verify) {
                ...
                while (offset < len) {
                    ...
                    s_err = stm32_read_memory(
                        stm, addr + offset, compare + offset, rlen);
                    if (s_err != STM32_ERR_OK) {
                        fprintf(stderr,
                ...
                for (r = 0; r < len; ++r)
                    if (buffer[r] != compare[r]) {
                        if (failed == retry) {
                            fprintf(stderr,
                                    "Failed to verify at address 0x%08x, "
...
int parse_options(int argc, char* argv[])
{
    ...
    while ((c = getopt(argc, argv, "a:b:m:r:w:e:vn:g:jkfcChuos:S:F:i:R")) !=
           -1) {
        switch (c) {
            case 'a': port_opts.bus_addr = strtoul(optarg, NULL, 0); break;
            ...
            case 'b':
                ...
                if (port_opts.baudRate == SERIAL_BAUD_INVALID) {
                    ...
                    for (baudrate = SERIAL_BAUD_1200;
                         baudrate != SERIAL_BAUD_INVALID;
                         ++baudrate)
            ...
            case 'e':
                if (readwrite_len || start_addr) {
                    fprintf(stderr,
                            "ERROR: Invalid options, can't specify start page "
                ...
                if (npages > STM32_MAX_PAGES || npages < 0) {
                    fprintf(stderr,
                            "ERROR: You need to specify a page count between 0 "
                            "and 255");
                    return 1;
                }
                if (!npages)
            ...
            case 'n': retry = strtoul(optarg, NULL, 0); break;
            ...
            case 'g':
                ...
                if (execute % 4 != 0) {
                    fprintf(stderr,
                            "ERROR: Execution address must be word-aligned\n");
                    return 1;
                }
                break;
            case 's':
            ...
            case 'S':
                if (spage || npages) {
                    ...
                    if (*pLen == ':') {
                        ...
                        if (readwrite_len == 0) {
                            fprintf(stderr,
                                    "ERROR: Invalid options, can't specify "
                                    "zero length\n");
                            return 1;
                        }
                    }
            ...
            case 'F':
                ...
                break;
            case 'f': force_binary = 1; break;

            case 'c': init_flag = 0; break;

            case 'h': show_help(argv[0]); exit(0);

            case 'i': gpio_seq = optarg; break;
    ...
    if ((action != ACT_WRITE) && verify) {
        fprintf(stderr,
                "ERROR: Invalid usage, -v is only valid when writing\n");
        show_help(argv[0]);
        return 1;
    }
...
void show_help(char* name)
{
    fprintf(
        ...
        "	-F RX_length[:TX_length]  Specify the max length of RX and TX "
        "frame\n"
        ...
        "\n"
        "	GPIO sequence:\n"
        "	- entry sequence: GPIO_3=low, GPIO_2=low, 100ms delay, "
        "GPIO_2=high\n"
        "	- exit sequence: GPIO_3=high, GPIO_2=low, 300ms delay, "
        "GPIO_2=high\n"

 ...
 * loader. This flag is used when BOOT0 pin is defining Main Flash memory as the
 * target boot space. When the flag is set, the device is considered as empty
 * and System memory (boot loader) is selected instead of the Main Flash as a
 * boot space to allow user to program the Flash memory. This flag is updated
 * only during Option bytes loading: it is set when the content of the address
 * 0x08000 0000 is read as 0xFFFF FFFF, otherwise it is cleared. It means a
 * power on or setting of OBL_LAUNCH bit in FLASH_CR register is needed to clear
 * this flag after programming of a virgin device to execute user code after
 * System reset.
 */
...
/* RM0394, Empty check
 * On STM32L452 (and possibly all STM32L45xxx/46xxx) internal empty check flag
 * is implemented to allow easy programming of the virgin devices by the boot
 * loader. This flag is used when BOOT0 pin is defining Main Flash memory as the
 * target boot space. When the flag is set, the device is considered as empty
 * and System memory (boot loader) is selected instead of the Main Flash as a
 * boot space to allow user to program the Flash memory. This flag is updated
 * only during Option bytes loading: it is set when the content of the address
 * 0x08000 0000 is read as 0xFFFF FFFF, otherwise it is cleared. It means a
 * power on or setting of OBL_LAUNCH bit in FLASH_CR register or a toggle of
 * PEMPTY bit in FLASH_SR register is needed to clear this flag after after
 * programming of a virgin device to execute user code after System reset. In
 * STM32L45xxx/46xxx the register FLASH_CR could be locked and a special SW
 * sequence is required for unlocking it. If a previous unsuccessful unlock has
 * happened, a reset is required before the unlock. Due to such complications,
 * toggling the PEMPTY bit in FLASH_SR seams the most reasonable choice. The
 * code below check first word in flash and flag PEMPTY. If they do not match,
 * then it toggles PEMPTY. At last, it resets.
 */
...
static const uint32_t stm_pempty_launch_code_length =
    sizeof(stm_pempty_launch_code);
                                              ...
                                              time_t         timeout)
{
    ...
    if (s_err == STM32_ERR_NACK)
        ...
        fprintf(
            stderr, "Unexpected reply from device on command 0x%02x\n", cmd);
    return STM32_ERR_UNKNOWN;
}
 ...
 */
static stm32_err_t stm32_guess_len_cmd(const stm32_t* stm,
                                       uint8_t        cmd,
                                       uint8_t*       data,
                                       unsigned int   len)
{
...
#define newer(prev, a) \
    (((prev) == STM32_CMD_ERR) ? (a) : (((prev) > (a)) ? (prev) : (a)))
...
stm32_t* stm32_init(struct port_interface* port, const char init)
{
    ...
    for (i = 1; i < len; i++) {
        val = buf[i + 1];
        switch (val) {
            case STM32_CMD_GET: stm->cmd->get = val; break;
            case STM32_CMD_GVR: stm->cmd->gvr = val; break;
            case STM32_CMD_GID: stm->cmd->gid = val; break;
            case STM32_CMD_RM: stm->cmd->rm = val; break;
            case STM32_CMD_GO: stm->cmd->go = val; break;
            case STM32_CMD_WM:
    ...
    if (stm->cmd->get == STM32_CMD_ERR || stm->cmd->gvr == STM32_CMD_ERR ||
        stm->cmd->gid == STM32_CMD_ERR) {
        fprintf(stderr,
                "Error: bootloader did not returned correct information from "
                "GET command\n");
        return NULL;
    }
    ...
    if (len < 2) {
        stm32_close(stm);
        fprintf(stderr,
                "Only %d bytes sent in the PID, unknown/unsupported device\n",
                len);
        return NULL;
    }
    ...
    stm->dev = devices;
    while (stm->dev->id != 0x00 && stm->dev->id != stm->pid) ++stm->dev;
...
stm32_err_t stm32_read_memory(const stm32_t* stm,
                              uint32_t       address,
                              uint8_t        data[],
                              unsigned int   len)
{
...
stm32_err_t stm32_write_memory(const stm32_t* stm,
                               uint32_t       address,
                               const uint8_t  data[],
                               unsigned int   len)
{
...
stm32_err_t stm32_runprot_memory(const stm32_t* stm)
{
    ...
    if (stm->cmd->ur == STM32_CMD_ERR) {
        fprintf(stderr,
                "Error: READOUT UNPROTECT command not implemented in "
                "bootloader.\n");
        return STM32_ERR_NO_CMD;
    }
...
static stm32_err_t stm32_mass_erase(const stm32_t* stm)
{
    ...
    if (s_err != STM32_ERR_OK) {
        fprintf(stderr,
                "Mass erase failed. Try specifying the number of pages to be "
                "erased.\n");
...
static stm32_err_t
    stm32_pages_erase(const stm32_t* stm, uint32_t spage, uint32_t pages)
{
    struct port_interface* port = stm->port;
    stm32_err_t            s_err;
    port_err_t             p_err;
     ...
     */
    /* 0x44 is Extended Erase, a 2 byte based protocol and needs to be handled
     * differently. */
    ...
    if (s_err != STM32_ERR_OK) {
        fprintf(stderr,
                "Page-by-page erase failed. Check the maximum pages your "
                "device supports.\n");
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->er != STM32_CMD_EE_NS)
            stm32_warn_stretching("erase");
        return STM32_ERR_UNKNOWN;
...
stm32_err_t
    stm32_erase_memory(const stm32_t* stm, uint32_t spage, uint32_t pages)
{
    uint32_t    n;
    stm32_err_t s_err;
                                      ...
                                      uint32_t       code_size)
{
    ...
    uint32_t code_address_le =
        le_u32(target_address + 8 + 1); // thumb mode address (!)
...
stm32_err_t stm32_reset_device(const stm32_t* stm)
{
    ...
    if (stm->dev->flags & F_OBLL) {
        ...
        return stm32_run_raw_code(stm,
                                  target_address,
                                  stm_obl_launch_code,
                                  stm_obl_launch_code_length);
    } else if (stm->dev->flags & F_PEMPTY) {
        /* clear the PEMPTY bit to reset the device (see RM0394) */
        return stm32_run_raw_code(stm,
                                  target_address,
                                  stm_pempty_launch_code,
                                  stm_pempty_launch_code_length);
    } else {
        return stm32_run_raw_code(
            stm, target_address, stm_reset_code, stm_reset_code_length);
    }
}

stm32_err_t stm32_crc_memory(const stm32_t* stm,
                             uint32_t       address,
                             uint32_t       length,
                             uint32_t*      crc)
{
...
stm32_err_t stm32_crc_wrapper(const stm32_t* stm,
                              uint32_t       address,
                              uint32_t       length,
                              uint32_t*      crc)
{

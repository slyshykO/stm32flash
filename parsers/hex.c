...
parser_err_t hex_open(void* storage, const char* filename, const char write)
{
    ...
    if (write) {
        ...
        while (read(fd, &mark, 1) != 0) {
            ...
            /* setup the checksum */
            checksum = reclen + ((address & 0xFF00) >> 8) +
                       ((address & 0x00FF) >> 0) + type;
            ...
            switch (type) {
                ...
                case 0:
                    ...
                    /* if there is a gap, set it to 0xff and increment the
                     * length */
                    if (c > 0) {
            ...
            switch (type) {
                /* EOF */
                case 1: close(fd); return PARSER_ERR_OK;
                ...
                case 2:
                    ...
                    if (len > st->data_len) {
                        ...
                        memset(
                            &st->data[st->data_len], 0xff, len - st->data_len);
                        st->data_len = len;
                    }
                    break;
            }
...
parser_t PARSER_HEX =
    {"Intel HEX", hex_init, hex_open, hex_close, hex_size, hex_read, hex_write};

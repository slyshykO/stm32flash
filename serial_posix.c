                               ...
                               const serial_stopbit_t stopbit)
{
    ...
    switch (baud) {
        ...
        default: return PORT_ERR_UNKNOWN;
    }
...
static port_err_t
    serial_posix_read(struct port_interface* port, void* buf, size_t nbyte)
{
    serial_t* h;
    ssize_t   r;
...
static port_err_t
    serial_posix_write(struct port_interface* port, void* buf, size_t nbyte)
{
    serial_t*      h;
    ssize_t        r;
...
static port_err_t
    serial_posix_gpio(struct port_interface* port, serial_gpio_t n, int level)
{
    serial_t* h;
    int       bit, lines;
    ...
    switch (n) {
        case GPIO_RTS: bit = TIOCM_RTS; break;

        case GPIO_DTR: bit = TIOCM_DTR; break;
